...
## JAN 2025.. TRYING TO WRITE THESE OUT IN ORDER, WITH ALL SI AND BONUS STUFF AT THE END


## Table 1: Data Summary (add individual BeSD questions to this one)
## NEXT: Individual BeSD questions results



## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Table 1 for the Manuscript. All regression variables described by outcome
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## New Figure 1 (JAN 2025): Each BeSD Variable and Outcome. 
...
for(i in 1:4){
  ...
  listofplots[[i]] <-
    ...
    theme(axis.text.y=element_text(size=18,lineheight = .5),
          axis.text.x=element_text(size=18,lineheight = 1,angle=45,hjust=1),
...
pdf('manu_output/figure 1 -- besd questions and vxstat.pdf',width=18,height=40,onefile=FALSE)
...
pdf('manu_output/figure 4 -- regression forest plot.pdf',width=27,height=19)
ggplot(
  rbind(
    zdtab,uitab
  )) +
...
# We tested several versions of these models and compared them on the basis of their variance e
# xplained (McFadden R-Squared).  The 5 priority questions and the LCA classes are two approaches 
# to reducing the long BeSD question set, one via shortening surveys, and the other via analysis, 
# and we wanted to test how much explanatory power is lost in these reductions. We ran twelve models 
# in total, each for zero-dose and under-vaccinated outcomes. We first ran models using the 16 BeSD 
# questions compared to models using the 5 priority BeSD questions as well as ones using the LCA-derived 
# domains. We also ran these with the full set of socio-demographic variables. 
...
pdf('figure SI -- LCA and province.pdf',width=16,height=27,onefile=FALSE)
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Pairwise correlation heatmaps of cllab variables

# thinkingfeeling and motivation
...
png('figure SI -- LCA and survey year.png',width=1200,height=800)
ggplot(ctmp) + 
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## SAME AS ABOVE BUT BY PROVINCE

tmptab <- tmp[, .(N=.N), by = c('question','value_label','vxstatus','besd_category_nice','province')]
tmptab[, pct := N/sum(N), by = c('question','value_label','province')]
tmptab[, labN := sum(N), by = c('question','value_label','province')]
tmptab[, vallab := paste0(value_label,'\n(N=',prettyNum(labN,big.mark=','),')')]


# plot 
for(p in c('tn Tanganyika Province','hl Haut Lomami Province')){ #,'ll Lualaba Province')){
  listofplots <- list()
  message(p)
  ...
  for(i in 1:4){
    ...
    listofplots[[i]] <-
      ...
      ggplot(tmptab[besd_category_nice==bcn & province==p]) +
      geom_bar(aes(vallab,pct,fill=vxstatus),stat='identity') +
      facet_wrap(~besd_category_nice+str_wrap(question, width = 40),scales='free_x') +
      scale_y_continuous(labels=scales::percent) +
      theme(axis.text.y=element_text(size=18,lineheight = .5),
            axis.text.x=element_text(size=18,lineheight = 1,angle=45,hjust=1),
            strip.text=element_text(size=18),
      ...
      ylab('') + #Latent Class Assigned') +
      xlab('') + #Question Response') +
      ggtitle(paste0(p,' - ',bcn)) +
      scale_fill_manual(values=rev(c('#FF6363','#F8B400','#125B50')),name='')
    
  }
  
  pdf(paste0('figure SI -- questions and vxstat by ',p,'.pdf'),width=20,height=55,onefile=FALSE)
  egg::ggarrange(listofplots[[1]],listofplots[[2]],listofplots[[3]],listofplots[[4]],ncol=1)
  dev.off()
}
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## distribution of motivation in ZD pop across BESD variables
...
pdf('figure SI -- LCA and motivation by BeSD question.pdf',width=15,height=25)
...
### ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Look at info source wrt to besd categories
...
pdf('figure SI -- info source and besd categories.pdf',width=20,height=30,onefile=FALSE)
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Corelation between demographics and LCA classes
...
## TODO: Look at uncertainty via posterior probibilities of class membership